...
const getImageUrl = (images: any[]) => {
    ...
    const medium = images.find((img: any) => img.size === 'medium')?.['#text'];
    return mega || extralarge || large || medium || null;
};
                        ...
                        {tracks.slice(0, 5).map((track: any, i: number) => {
                            ...
                            return (
                                <li key={`${track.name}-${i}`} className="flex items-center gap-3 group">
                                    ...
                                    <div className="relative w-9 h-9 rounded overflow-hidden bg-white/10 shrink-0 shadow-lg">
                                        {trackImg && (
                                            <Image src={trackImg} alt={track.name} fill className="object-cover" unoptimized />
                                        )}
                                    </div>

                                    <div className="flex flex-col min-w-0 justify-center">
                                        {/* AJUSTE FINO: text-[11px] e removido truncate */}
            ...
            <div className="z-10 pt-4 border-t border-white/10 mt-auto">
                <div className="flex justify-between items-end">
                    ...
                    <div className="text-right">
                        <p className="text-[10px] text-neutral-500 uppercase tracking-wider mb-0.5">Scrobbles</p>
