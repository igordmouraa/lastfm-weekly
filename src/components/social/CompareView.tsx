...
function UserAvatar({ name }: { name: string }) {
    ...
    const initial = name.trim()[0]?.toUpperCase() ?? tc('unknownInitial');
    return (
    ...
    return (
        ...
        >
            <div className="flex items-center gap-3">
                <UserAvatar name={value || tc('unknownInitial')} />
...
export function CompareView({ user1: initialUser1, user2: initialUser2, result }: CompareViewProps) {
    ...
    const user1 = user1Override ?? initialUser1 ?? currentUser ?? '';
    ...
    return (
        ...
        >
            <motion.header variants={fadeUp} className="text-center">
                ...
                <h1 className="text-3xl font-display font-bold tracking-tight">{t('compare.title')}</h1>
                <p className="text-sm text-neutral-500 mt-2 max-w-md mx-auto">{t('compare.subtitle')}</p>
            </motion.header>
            ...
            <motion.form variants={fadeUp} onSubmit={handleSubmit} className="space-y-5">
                <div className="relative flex flex-col sm:flex-row sm:items-stretch">
                    <UserSlot
                        label={t('compare.profileA')}
                        value={user1}
                        onChange={setUser1Override}
                        placeholder={t('compare.placeholderYourUser')}
                        accent="left"
                    />
                    <div className="hidden sm:flex items-center justify-center w-12 shrink-0 relative z-10 -mx-1">
                        <div className="w-10 h-10 rounded-full bg-neutral-950 border border-white/10 flex items-center justify-center">
                            <span className="text-[10px] font-black text-neutral-500">{tc('vs')}</span>
                        </div>
                    </div>
                    <div className="sm:hidden flex items-center justify-center py-2">
                        <span className="text-xs font-bold text-neutral-600">{tc('vsLower')}</span>
                    </div>
                    <UserSlot
                        label={t('compare.profileB')}
                        value={user2}
                        onChange={setUser2Override}
                        placeholder={t('compare.placeholderOtherUser')}
                        accent="right"
                    />
                </div>
                ...
                <motion.section variants={fadeUp} className="space-y-8 pt-4 border-t border-white/[0.06]">
                    <div className="text-center space-y-4">
                        ...
                        <p className="text-7xl font-display font-bold text-red-400 tabular-nums leading-none">
                            {t('compare.overlapPercent', { percent: result.overlapPercent })}
                        </p>
                        <p className="text-sm text-neutral-500">{tc('overlap')}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 text-center">
                        <div className="py-3">
                            ...
                            <p className="text-[10px] uppercase tracking-widest text-neutral-600 mt-1">{tc('inCommon')}</p>
                        </div>
                        <div className="py-3 border-x border-white/[0.06]">
                            ...
                            <p className="text-[10px] uppercase tracking-widest text-neutral-600 mt-1 truncate px-1">
                                {tc('onlyUser', { username: result.user1 })}
                            </p>
                        </div>
                        <div className="py-3">
                            <p className="text-2xl font-display font-bold tabular-nums text-neutral-400">
                            ...
                            <p className="text-[10px] uppercase tracking-widest text-neutral-600 mt-1 truncate px-1">
                                {tc('onlyUser', { username: result.user2 })}
                            </p>
                        </div>
                    </div>
