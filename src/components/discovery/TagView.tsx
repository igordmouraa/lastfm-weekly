...
export function TagView({ tag, artists, albums }: TagViewProps) {
    const t = useTranslations('discovery.tag');
            ...
            <motion.div variants={fadeUp} className="grid lg:grid-cols-2 gap-x-14 gap-y-10">
                <section>
                    <div className="flex items-center gap-2 mb-5">
                        ...
                        <h2 className="text-xs font-bold uppercase tracking-widest text-violet-400">{t('artists')}</h2>
                    </div>
                ...
                <section>
                    <div className="flex items-center gap-2 mb-5">
                        ...
                        <h2 className="text-xs font-bold uppercase tracking-widest text-emerald-400">{t('albums')}</h2>
                    </div>
                    {albumItems.length > 0 ? (
                        <RankedList items={albumItems} accent="#34d399" />
                    ) : (
                        <p className="text-sm text-neutral-600">{t('noAlbums')}</p>
                    )}
                </section>
            </motion.div>
        </motion.div>
